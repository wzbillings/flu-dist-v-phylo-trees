###
# Grantham amino-acid distances
# Zane Billings
###

grantham_substitution_matrix <- function() {
	amino_acids <- c(
		"A", "R", "N", "D", "C", "Q", "E", "G", "H", "I",
		"L", "K", "M", "F", "P", "S", "T", "W", "Y", "V"
	)
...
validate_aligned_sequence_pair <- function(seq_1, seq_2) {
	if (!is.character(seq_1) || length(seq_1) != 1 || is.na(seq_1)) {
		stop("seq_1 must be a length-one character string and must not be missing.", call. = FALSE)
	}
	if (!is.character(seq_2) || length(seq_2) != 1 || is.na(seq_2)) {
		stop("seq_2 must be a length-one character string and must not be missing.", call. = FALSE)
	}
	if (nchar(seq_1, type = "bytes") != nchar(seq_2, type = "bytes")) {
...
grantham_pair_distance <- function(seq_1, seq_2, matrix = grantham_substitution_matrix()) {
	...
	scores <- matrix[cbind(residues_1[comparable], residues_2[comparable])]
	mean(scores)
}
...
dist_grantham <- function(seqs) {
	...
	if (length(seqs) > 1) {
		substitution_matrix <- grantham_substitution_matrix()
		for (i in 2:nrow(res)) {
			for (j in 1:(i - 1)) {
...
calculate_grantham_distance <- function(aligned_proteins) {
	dist_grantham(aligned_proteins)
}
